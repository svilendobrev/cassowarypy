...
class Strength {
 ...
 public:
  ...
  Strength( const string & Name, const SymbolicWeight & symbolicWeight) :
    _name( Name), _symbolicWeight( symbolicWeight)
    { }

  // special case for when nLevels = 3, should assert nLevels() == 3
  Strength( const string & Name, double w1, double w2, double w3);

  virtual ~Strength()
    { }

  virtual bool IsRequired() const
    { return ( _symbolicWeight == sRequired()._symbolicWeight); }
