// $Id: FDBinaryOneWayConstraint.cc,v 1.1.1.1 2003/01/15 14:06:20 svilen Exp $
//
...
void 
FDBinaryOneWayConstraint::EnsurePreconditionsForCn( const Constraint & cn)
{
  VarSet setRO = cn.ReadOnlyVars();
  if ( setRO.size() > 1) 
    throw ExCLTooDifficultSpecial("Only 0 or 1 read only variables are allowed");
  LinearExpression expr = cn.Expression();
  ...
  Variable clv = (*terms.begin()).first;
...
bool 
FDBinaryOneWayConstraint::FCanConvertCn( const Constraint & cn)
{
  try {
...
FDBinaryOneWayConstraint::FDBinaryOneWayConstraint( const Constraint & cn)
    :FDConstraint( cn.strength(), cn.weight())
{
  EnsurePreconditionsForCn( cn);
  list<FDNumber> l;
  ...
  Variable clvRO = clvNil;
  Variable clvROLinear = clvNil;
  Number coeffRO = 0;

  Variable clvRW = clvNil;
  Number coeffRW = 0;

  if ( setRO.size() == 1) {
    const Variable & clv = * ( setRO.begin());
    if ( clv->IsFDVariable())
      clvRO = clv;
    else
      clvRO = new FDVariable( clv.Name(),clv.IntValue(),l);
    clvROLinear = clv;
  }
  LinearExpression expr = cn.Expression();
  const VarToNumberMap & terms = expr.Terms();

  for ( VarToNumberMap::const_iterator it = terms.begin();
       it != terms.end();
       ++it) {
    ...
    if ( clv == clvROLinear) {
      ...
      if ( clv->IsFDVariable())
        ...
        clvRW = new FDVariable( clv.Name(),clv.Value(),l);
      coeffRW = (*it).second;
    }
  }
  assert(!clvRW.IsNil());
